...
export interface SalariosMensais {
  salario_talyson?: number;
  salario_mikaelly?: number;
}

export async function updateSalarios(
  id: number | string,
  salarios: SalariosMensais
) {
  const updateData: Record<string, number> = {};

  if (salarios.salario_talyson !== undefined) {
    updateData.salario_talyson = salarios.salario_talyson;
  }

  if (salarios.salario_mikaelly !== undefined) {
    updateData.salario_mikaelly = salarios.salario_mikaelly;
  }
  ...
  if (error) {
    const errorMessage =
      ...
      'Erro desconhecido ao atualizar salários';
